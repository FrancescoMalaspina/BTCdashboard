...
col1 = dbc.Col([
    ...
    dbc.InputGroup([
        ...
        dbc.Input(type="number", min=1, value=100, id="input-S")
    ]),
    dbc.Label("Strike price"),
    dbc.InputGroup([
        dbc.InputGroupText("X"),
        dbc.Input(type="number", min=1, value=100, id="input-X")
    ]),
    dbc.Label("Periods to maturity"),
    dbc.InputGroup([
        dbc.InputGroupText("T"),
        dbc.Input(type="number", min=1, step=1, value=365, id="input-T")
    ]),
    dbc.Label("Risk free interest rate"),
    dbc.InputGroup([
        dbc.InputGroupText("r"),
        dbc.Input(type="number", min=1e-3, value=0.03, id="input-r")
    ]),
    dbc.Label("Volatility"),
    dbc.InputGroup([
        dbc.InputGroupText("v"),
        dbc.Input(type="number", min=1e-3, value=10, id="input-v")
    ]),
    html.Br(),
    call,
], width=4)
